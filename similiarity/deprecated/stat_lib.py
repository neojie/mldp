...
def analyze_single(idx,ase_xyz,mda_xyz, project_axis,solid_center0=-1,alpha = 2.5,plot=True,verbose=False, debug=False, assert_chi = True):
    ...
    return chs, chl,chw, ls, ll, w,hs, hl , hw, z0, z1_unpbc, result.redchi, mgs,mgl,mgw,sis,sil,siw,os,ol,ow    
    
def analyze_single_h(idx,ase_xyz,mda_xyz, project_axis,solid_center0=-1,alpha = 2.5,plot=True,verbose=False, debug=False, assert_chi = True):
    """
    Analyze hydrogen only
...
def analyze_h(start_idx,en_idx, filepath,ase_xyz,mda_xyz, project_axis,alpha, step=1,save=True,name='stat.txt', assert_chi=True):
    """
...
def analyze(start_idx,en_idx, filepath,ase_xyz,mda_xyz, project_axis,alpha, step=1,save=True,name='stat.txt', assert_chi=True):
    ...
    ch = np.zeros((size,22))  # 
    for fn in range(start_idx,en_idx,step):
        chs, chl,chw, ls, ll, w, hs, hl, hw,z0,z1_unpbc,chi,mgs,mgl,mgw,sis,sil,siw,os,ol,ow = analyze_single(fn,ase_xyz,mda_xyz, project_axis,alpha=alpha,plot=False,assert_chi=assert_chi)
        idx = (fn - start_idx)//step
        ch[idx,0] = fn
        ch[idx,1], ch[idx,2], ch[idx,3] = chs, chl, chw
        ch[idx,4], ch[idx,5], ch[idx,6] = ls, ll, w
        ...
        ch[idx,10], ch[idx,11] = z0,z1_unpbc
        ch[idx,12] = chi
        ch[idx,13],ch[idx,14], ch[idx,15] = mgs,mgl,mgw
        ch[idx,16],ch[idx,17], ch[idx,18] = sis,sil,siw
        ch[idx,19],ch[idx,20], ch[idx,21] = os,ol,ow
...
def save_ch(ch,start_idx, alpha, filepath,name):
    ...
    """
    fmt = '%d %.4f %.4f %.4f %.4f %.4f %.4f %d %d %d %.4f %.4f %.4f %d %d %d %d %d %d %d %d %d'
    np.savetxt(name,ch,fmt=fmt,
                header='{2} \n python start_idx = {0} alpha = {1} dim = ls+ll+w*2 \n id,cs,cl,cw,ls,ll.lw,hs,hl,hw,z0,z1_unpbc,reduced_chi, mgs,mgl,mgw,sis,sil,siw,os,ol,ow'.format(start_idx, alpha, filepath)) 
    
def save_ch_h(ch,start_idx, alpha, filepath,name):
    """
    # cs (H content in solid), cl (H content in liquid), cw (H in interface)
    # ls (solid slab length),ll (liquid slab length),w (interface), 
    # hs (# of H in solid), hl (# of H in liquid),hw (# of H in inteface)
