...
#        coor   = np.load(os.path.join(dire,'coord.npy'))
        ...
        natoms = force.shape[-1]//3
        natoms_ref =  160   # the scale model is based on 160 atoms
        
        vol_ref = vol/natoms*natoms_ref  # scale system to natoms_ref

        scale  = scale_model(vol_ref, fparam[0])
