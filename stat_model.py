...
### save log file
    ...
    log.writeline ("# number of test data : %d " % num_tr)
    log.writeline ("# Energy L2err        : %e eV" % l2e_tr)
    log.writeline ("# Energy L2err/Natoms : %e eV" % l2ea_tr)
    log.writeline ("# Force  L2err        : %e eV/A" % l2f_tr)
    log.writeline ("# Virial L2err        : %e eV" % l2v_tr)
    log.writeline ("# Virial L2err/Natoms : %e eV" % l2v_tr/natoms)
    log.writelines([str(i) for i in [num_tr,l2e_tr,l2ea_tr,l2f_tr,l2v_tr,l2v_tr/natoms]])
    log.writeline('## train results stored at {0}.*.out##'.format(args.detail_file))
    log.writeline ("# number of test data : %d " % num_test)
    log.writeline ("# Energy L2err        : %e eV" % l2e_test)
    log.writeline ("# Energy L2err/Natoms : %e eV" % l2ea_test)
    log.writeline ("# Force  L2err        : %e eV/A" % l2f_test)
    log.writeline ("# Virial L2err        : %e eV" % l2v_test)
    log.writeline ("# Virial L2err/Natoms : %e eV" % l2v_test/natoms)  
