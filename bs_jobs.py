...
###
vols = np.array([1500, 1114.5, 939.1, 738.63, 518.76])
...
def build_folder(vol):
    scale = (vol/vol0)**(1/3)
    folder_name = os.path.join(init_path,'r1-'+str(int(vol)))
...
if len(existing_vols) >0:  
    for vol in vols:
        ...
        command = 'cd ' + folder +';python /gpfs/loomis/home.grace/jd848/script/petv/check_input.py' +';' 'sbatch -J {0} sub_job.sh'.format(vol)+ ';' + 'cd ' + init_path
