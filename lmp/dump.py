...
#   09/21, Jie Deng: improve python 3 compatibility, modify code on list index
...
#    self.snaps.sort(self.compare_time)=>JD
    self.cull()
    ...
    if (not self.names.__contains__("x")) or \
       (not self.names.__contains__("y")) or \
       (not self.names.__contains__("z")):
  ...
  def names2str(self):
    ...
    str = ""
    for i in range(ncol):
      if i in values: str += list(pairs)[list(values).index(i)][0] + ' '
    return str
  ...
  def set(self,eq):
    ...
    lhs = list[0][1:]
    if not self.names.__contains__(lhs):
      self.newcolumn(lhs)
  ...
  def setv(self,colname,vec):
    print("Setting ...")
    if not self.names.__contains__(colname):
  ...
  def spread(self,old,n,new):
    iold = self.names[old]
    if not self.names.__contains__(new): self.newcolumn(new)
    inew = self.names[new]
