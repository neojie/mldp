...
def analyze_single(idx,ase_xyz,mda_xyz, project_axis,solid_center0=-1,alpha = 2.5,plot=True,verbose=False, debug=False, assert_chi = True,mode='mass'):
    """
    analyze all species
    ...
    inter, k,ase_a,mda_a = cal_inter(ase_xyz,mda_xyz,idx,mesh=1., alpha=alpha, level=None,mode=mode)
...
def analyze(start_idx,en_idx, filepath,ase_xyz,mda_xyz, project_axis,alpha, step=1,save=True,name='stat.txt', assert_chi=True,mode='mass'):
    """
